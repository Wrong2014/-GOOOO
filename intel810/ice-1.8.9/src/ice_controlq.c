		       ...
		       struct ice_sq_cd *cd)
{
	...
	if (!cq->sq.count) {/* 发送队列为空 */
		ice_debug(hw, ICE_DBG_AQ_MSG, "Control Send queue not initialized.\n");
		status = ICE_ERR_AQ_EMPTY;
		goto sq_send_command_error;
	}

	if ((buf && !buf_size) || (!buf && buf_size)) {/* buf为空或者buf_size为空 */
		status = ICE_ERR_PARAM;
		goto sq_send_command_error;
	}

	if (buf) {
		if (buf_size > cq->sq_buf_size) {/* 发送的buf的size大于发送队列buf的大小 */
    ...
    //得到发送队列的头，循环队列的val值理论最大上是队列深度-1，不可能出现等于或者大于的情
    //况，出现则为异常情况
	 ...
	 */
	 /* 调用清理和检查队列可用函数来回收由 FW/MBX 处理的描述符； 
	 该函数返回可用的描述数*/
    ...
    /* next_to_use累加，该值的范围在0-cq->sq.count - 1之间 */
	(cq->sq.next_to_use)++;
    /* 当cq->sq.next_to_use = cq->sq.count , 下标值 = 索引值%队列深度，当两个值相等，
        说明循环队列转了一圈，从新从下标值0开始 */
	...
	do {
		...
		udelay(ICE_CTL_Q_SQ_CMD_USEC);//延迟100微秒
		total_delay++;
	} while (total_delay < cq->sq_cmd_timeout);//最大延迟250毫秒
	...
	if (ice_sq_done(hw, cq)) {
		...
		cmd_completed = true;//处理完成设置为true
