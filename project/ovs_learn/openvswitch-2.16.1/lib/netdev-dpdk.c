...
{
    /*mtu*/
    uint32_t buf_size = dpdk_buf_size(dev->requested_mtu);/*mtu*/
    struct dpdk_mp *dmp;/*dpdk内存池*/
    int ret = 0;
    bool per_port_mp = dpdk_per_port_memory();/*每端口内存池*/
    ...
    OVS_REQUIRES(dev->mutex)
{
    ...
    struct rte_eth_dev_info info;/*dev info*/
    ...
    OVS_REQUIRES(dpdk_mutex)
{
    ...
    netdev->n_rxq = 0;
    netdev->n_txq = 0;
    dev->requested_n_rxq = NR_QUEUE;/*配置端口的rxq个数 1*/
