                    ...
                    const uint32_t bit_count_u1)
{
     ...
     */
     /* block scratch是在进行规则验证时重用相同数据包minflow块数据的一种优化。
        这减少了查找期间所做的工作，从而提高了性能。
        blocks_scratch数组被存储为一个线程局部变量，因为每个线程都需要自己的块内存。*/
    ...
    ULLONG_FOR_EACH_1 (i, found_map) {
        ...
        //上面只是根据hash查找，返回值nodes可能是匹配hash值的一个集合，
        //这里遍历nodes[i]找到真正匹配的流表
