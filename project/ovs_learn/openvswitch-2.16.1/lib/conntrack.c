...
conntrack_init(void)
{
    ...
    atomic_init(&ct->n_conn_limit, DEFAULT_N_CONN_LIMIT);/*conn 限制个数*/
...
/*******************************************************************************
 函数名称  :    ct_sweep
 功能描述  :    从ct bucket上删除超时的conn，返回现有链接最早的超时时间，
                返回值可能不是一个未来的时间，比当前时间还小，因为到了要返回的限制数
 输入参数  :    ct---链接跟踪
               now---当前时间
               limit---最大要老化的链接数
 输出参数  :    
...
ct_sweep(struct conntrack *ct, long long now, size_t limit)
{
    ...
    long long min_expiration = LLONG_MAX;/*最早的超时时间*/
    size_t count = 0;/*超时的ct计数*/
    ...
    for (unsigned i = 0; i < N_CT_TM; i++) {
        LIST_FOR_EACH_SAFE (conn, next, exp_node, &ct->exp_lists[i]) {
            ...
            if (now < conn->expiration || count >= limit) {
                ...
                ovs_mutex_unlock(&conn->lock);
                if (count >= limit) {/*超时个数已经超出了限制的10%，直接返回链表首*/
