    ...
    OVS_REQUIRES(dp_netdev_mutex)
{
    ...
    struct dp_netdev *dp;/*dp对应的netdev*/
    int error;
    ...
    dp = xzalloc(sizeof *dp);/*申请dp 结构内存*/
    shash_add(&dp_netdevs, name, dp);/*挂到全局dp_netdev链表，name是ovs-netdev*/

    *CONST_CAST(const struct dpif_class **, &dp->class) = class;/*dp赋值和初始化*/
    /*backer对应 dp_netdev的name 为ovs-netdev*/
    *CONST_CAST(const char **, &dp->name) = xstrdup(name);
    ovs_refcount_init(&dp->ref_cnt);/*dp引用次数*/
    atomic_flag_clear(&dp->destroyed);

    ovs_mutex_init_recursive(&dp->port_mutex);
    hmap_init(&dp->ports);/*dp_netdev下port初始化*/
    ...
    dp->reconfigure_seq = seq_create();/*配置序列号*/
            ...
            odp_port_t port_no, struct dp_netdev_port **portp)
{
    ...
    *portp = NULL;
    ...
    port->need_reconfigure = true;/*需要重新配置网络设备*/
...
{
    struct netdev_saved_flags *sf;
    struct dp_netdev_port *port;/*要添加的端口*/
    int error;
    ...
    seq_change(dp->port_seq);/*序列号改变 序列号+1*/

    reconfigure_datapath(dp);/*dp端口有变化重新配置网桥*/
...
dpif_netdev_port_add(struct dpif *dpif, struct netdev *netdev,
                     odp_port_t *port_nop)
{
    struct dp_netdev *dp = get_dp_netdev(dpif);/*获取数据面dp结构*/
    char namebuf[NETDEV_VPORT_NAME_BUFSIZE];/*获取数据面dp结构*/
    const char *dpif_port;/*数据面结构端口*/
...
{
    struct dpcls *cls;
    uint32_t hash = hash_port_no(in_port);/*匹配端口哈希值查找dpcls*/
...
megaflow_to_mark_find(const ovs_u128 *mega_ufid)
{
    ...
    /*全局mark链表查询*/
    CMAP_FOR_EACH_WITH_HASH (data, node, hash, &flow_mark.megaflow_to_mark) {
        if (ovs_u128_equals(*mega_ufid, data->mega_ufid)) {/*匹配到mega ufid*/
            return data->mark;
        }
    }
...
{
    struct dp_netdev_pmd_thread *pmd = offload->pmd;/*pmd*/
    struct dp_netdev_flow *flow = offload->flow;/*offload item对应的flow*/
    odp_port_t in_port = flow->flow.in_port.odp_port;/*in port端口*/
    const char *dpif_type_str = dpif_normalize_type(pmd->dp->class->type);
           ...
           uint32_t hash)
{
    ...
    for (i = 0; i < SMC_ENTRY_PER_BUCKET; i++) {
        if (bucket->flow_idx[i] == UINT16_MAX) {/*根据index 返回smc流表项*/
            bucket->sig[i] = sig;
            bucket->flow_idx[i] = index;
            return;
        }
    ...
    OVS_REQUIRES(pmd->flow_mutex)
{
    ...
    struct dp_netdev_flow *flow;
    struct netdev_flow_key mask;/*miniflow key 掩码结构*/
    struct dpcls *cls;/*port的dpcls*/
    size_t unit;
                           ...
                           struct dpif_flow *flows, int max_flows)
{
    ...
    struct dpif_netdev *dpif = dpif_netdev_cast(thread->up.dpif);/*获取dpif结构*/
    /*获取dp_netdev*/
    ...
    if (!dump->status) {
        ...
        int flow_limit = MIN(max_flows, FLOW_DUMP_MAX_BATCH);/*每次dump 50条flow*/
                                   ...
                                   struct tx_port *p)
{
    ...
    /*发完报文有剩余报错      */
    ovs_assert(pmd->n_output_batches > 0); /* 必须一次性全发完 */
    pmd->n_output_batches--;/*发出后批处理报文计数--*/

    pmd_perf_update_counter(&pmd->perf_stats, PMD_STAT_SENT_PKTS, output_cnt);/*发出去的包流量统计*/
    pmd_perf_update_counter(&pmd->perf_stats, PMD_STAT_SENT_BATCHES, 1);/*批处理发包流量统计*/
...
dp_netdev_process_rxq_port(struct dp_netdev_pmd_thread *pmd,
                           ...
                           odp_port_t port_no)
{
    struct pmd_perf_stats *s = &pmd->perf_stats;   /*流量统计*/
    struct dp_packet_batch batch;  /*队列报文批处理缓存结构*/
    struct cycle_timer timer;   /*循环定时器*/
    int error;
    int batch_cnt = 0; /*队列批处理缓存报文计数*/
    int rem_qlen = 0, *qlen_p = NULL;  /*队列中当前报文个数*/
    uint64_t cycles;
    ...
    pmd->ctx.last_rxq = rxq;    /*记录最近读取的队列*/
    dp_packet_batch_init(&batch);    /*报文批处理结构初始化*/
    ...
    if (pmd_perf_metrics_enabled(pmd) && rxq->is_vhost) {
        qlen_p = &rem_qlen;    /*当前队列报文个数、队列长度记录*/
    }
    /*从接收队列收包放入批处理结构，队列报文处理的都是批处理的结构*/
    error = netdev_rxq_recv(rxq->rx, &batch, qlen_p);   /* 调用对应的收包回调函数 */
    if (!error) {     /* poll 循环获取到了报文 */
        /* At least one packet received. */
        *recirc_depth_get() = 0;
        pmd_thread_ctx_time_update(pmd);    /*pmd线程上下文更新当前时间*/
        batch_cnt = dp_packet_batch_size(&batch);    /*队列批处理报文计数，从队列poll出的报文先放入批处理*/
        if (pmd_perf_metrics_enabled(pmd)) {    /*pmd 流量统计开关使能*/
            /* Update batch histogram. */
            s->current.batches++;    /*当前队列流量统计，队列批处理缓存的报文*/
            histogram_add_sample(&s->pkts_per_batch, batch_cnt);/*批处理报文数矩形图添加*/
            /* Update the maximum vhost rx queue fill level. */
            if (rxq->is_vhost && rem_qlen >= 0) {    /*队列为虚拟主机队列：记录当前队列报文个数、队列长度*/
                uint32_t qfill = batch_cnt + rem_qlen;
                if (qfill > s->current.max_vhost_qfill) {
                    s->current.max_vhost_qfill = qfill;/*虚拟主机队列报文数重填*/
                }
            }
        }

        /* Process packet batch. */
        /* dp_netdev_input_outer_avx512和dp_netdev_input 回调，默认后者 */
        int ret = pmd->netdev_input_func(pmd, &batch, port_no);
        if (ret) {
            /*将批处理batch中的包转入datapath 查emc、dpcls等操作，
                查找flow 、action，并且发送报文，前后都有计时*/
            dp_netdev_input(pmd, &batch, port_no);
        }

        /* Assign processing cycles to rx queue. */
        cycles = cycle_timer_stop(&pmd->perf_stats, &timer);/*接收队列处理循环停止    返回时钟周期*/
        dp_netdev_rxq_add_cycles(rxq, RXQ_CYCLES_PROC_CURR, cycles);  /*接收队列添加循环计数*/

        dp_netdev_pmd_flush_output_packets(pmd, false);/*pmd flush掉 pmd output pkt 要发的报文*/
    } else {
        /* Discard cycles. */
        cycle_timer_stop(&pmd->perf_stats, &timer);/*丢弃循环*/
        ...
        if (OVS_LIKELY(flow_node != NULL)) {
            ...
            CMAP_NODE_FOR_EACH (flow, node, flow_node) {
                ...
                flow->flow.in_port.odp_port == packet->md.in_port.odp_port)) {
                    ...
                    keys[i].len =
                        netdev_flow_key_size(miniflow_n_values(&keys[i].mf));/*获取miniflow key的size*/
                    emc_probabilistic_insert(pmd, &keys[i], flow);
                    /* Add these packets into the flow map in the same order
                     * as received.
                     */
                    packet_enqueue_to_flow_map(packet, flow, tcp_flags,
                                               flow_map, recv_idx);/*命中流表的报文填入每流报文批处理*/
                    n_smc_hit++;
                    hit = true;
                    break;
                }
 ...
 */
  /*******************************************************************************
   函数名称  :	  dfc_processing
   功能描述  :	  emc流表查询、emc流表未命中查smc流表、命中smc流表插入emc流表、
                   返回未命中的报文队列批处理结构 去查dpcls
...
{
    struct netdev_flow_key *key = &keys[0];    /*miniflow key 地址*/
    size_t n_missed = 0, n_emc_hit = 0, n_phwol_hit = 0,  n_mfex_opt_hit = 0;/* 相关计数 */
    struct dfc_cache *cache = &pmd->flow_cache;    /*pmd线程缓存索引、emc cached精确流表*/
    struct dp_packet *packet;
    const size_t cnt = dp_packet_batch_size(packets_);/*当前队列报文批处理缓存报文数*/
    ...
    DP_PACKET_BATCH_REFILL_FOR_EACH (i, cnt, packet, packets_) {
        ...
        /*metadata没有生效则初始化metadata，首先将pkt_metadata中flow_in_port前的字节全部设为0，
           然后将in_port.odp_port设为port_no,tunnel.ip_dst设为0，tunnel中的其他字段 也初始化0 */
        if (!md_is_valid) {
        ...
        if (netdev_flow_api && recirc_depth == 0) {
            ...
            if (OVS_LIKELY(flow)) {
                tcp_flags = parse_tcp_flags(packet);  /*从队列批处理报文提取tcp 标记*/
                n_phwol_hit++;
        ...
        key->hash =
                (md_is_valid == false)
                ? dpif_netdev_packet_get_rss_hash_orig_pkt(packet, &key->mf) /*从dpdk rss 或根据miniflow五元组算哈希*/
                : dpif_netdev_packet_get_rss_hash(packet, &key->mf); /*根据报文获取rss hash*/

        /* If EMC is disabled skip emc_lookup */
       /*emc查询，从hash表中查找，并且进行key值比较，
         根据key->hash，emc_entry alive, miniflow 3个条件得到dp_netdev_flow*/
        flow = (cur_min != 0) ? emc_lookup(&cache->emc_cache, key) : NULL;
        if (OVS_LIKELY(flow)) {
            tcp_flags = miniflow_get_tcp_flags(&key->mf);/*从miniflow提取tcp_flags*/
            n_emc_hit++;
            if (OVS_LIKELY(batch_enable)) {
                /* 如果匹配，调用dp_netdev_queue_batches将报文添加在flow->batches中，
                不匹配将不匹配的报文当前排。根据dp_netdev_flow对dp_packet分类，
                将同以个dp_netdev_flow对应的所有dp_packet
                放入相同的packet_batch_per_flow */
             ...
             * which will be returned to the caller for future processing. */
             /*记录未命中的报文key*/
            missed_keys[n_missed] = key;  /*记录未命中的报文key*/
            key = &keys[++n_missed];  /* 未命中的报文记录下来作为smc查找的入参 */
    ...
    *n_flows = map_cnt;
    ...
    if (!pmd->ctx.smc_enable_db) {
        return dp_packet_batch_size(packets_);/*返回队列批处理报文数     */
    }
                     ...
                     odp_port_t in_port)
{
    ...
    if (OVS_LIKELY(cls)) {
        /*dpcls查询
         1.通过classifier查找子流表，如果所有的报文都找到了匹配的子流表，将流表插入emc缓存中，
            并且将报文加入flow->batches。
         2.如果不匹配，则上报到controller。
         3.统计匹配、不匹配和丢失。
        */
    ...
    if (OVS_UNLIKELY(any_miss) && !fat_rwlock_tryrdlock(&dp->upcall_rwlock)) {
        ...
        DP_PACKET_BATCH_FOR_EACH (i, packet, packets_) {
             ...
             * to catch it here than execute a miss. */
             /*根据keys中的miniflow得到in_port，利用该in_port查找dpcls，
                若找到就调用dpcls_lookup在进行一次rule的查找*/
                  ...
                  bool md_is_valid, odp_port_t port_no)
{
    ...
    n_batches = 0;
/* 1.将dp_packet_batch中的所有包送入EMC(pmd->flow_cache)处理，IP 5元组的精确匹配，
      返回未命中emc要被送入fast_path_processing中处理的包数
   2.同时若md_is_valid为false该函数还将根据port_no初始化metadata
*/

/*1.将收到的报文解析key值，并且查emc流表，cache中，匹配的报文放入流表批处理，返回不匹配的报文个数
  2.如果存在不匹配的报文，调用fast_path_processing则继续查找dpcls全部表项，找到则将流表放入emc cache，
    不匹配则上报到controller*/
    ...
    /*快转路径处理，如果存在不匹配的报文，则调用fast_path_processing继续查找全部表项，
    找到则将流表放入cache，不匹配则上报到controller*/
...
const struct dpif_class dpif_netdev_class = {
    ...
    dpif_netdev_init,     //init
    dpif_netdev_enumerate,  //enumerate
    dpif_netdev_port_open_type,//port_open_type
    dpif_netdev_open,
    dpif_netdev_close,
    dpif_netdev_destroy,//destroy
    dpif_netdev_run,
    dpif_netdev_wait,
    dpif_netdev_get_stats,//get_stats
    NULL,                      /* set_features */
    dpif_netdev_port_add,//port_add
    dpif_netdev_port_del,
    dpif_netdev_port_set_config,//port_set_config
    dpif_netdev_port_query_by_number,
    dpif_netdev_port_query_by_name,
    NULL,                       /* port_get_pid */
    dpif_netdev_port_dump_start,//port_dump_start
    dpif_netdev_port_dump_next,
    dpif_netdev_port_dump_done,
    dpif_netdev_port_poll,//port_poll
    dpif_netdev_port_poll_wait,
    dpif_netdev_flow_flush,
    dpif_netdev_flow_dump_create,//flow_dump_create
    dpif_netdev_flow_dump_destroy,
    dpif_netdev_flow_dump_thread_create,//flow_dump_thread_create
    dpif_netdev_flow_dump_thread_destroy,//flow_dump_thread_destroy
    dpif_netdev_flow_dump_next,
    dpif_netdev_operate,//operate
    NULL,                       /* recv_set */
    NULL,                       /* handlers_set */
    ...
    dpif_netdev_register_upcall_cb, //flow_dump_next
...
dpcls_create_subtable(struct dpcls *cls, const struct netdev_flow_key *mask)
{
    ...
    cmap_init(&subtable->rules);/*掩码规则初始化*/
    subtable->hit_cnt = 0;
    netdev_flow_key_clone(&subtable->mask, mask);  /*掩码填入*/
    ...
    pvector_insert(&cls->subtables, subtable, 0);/* 按优先级插入后publish */
...
dpcls_sort_subtable_vector(struct dpcls *cls)
{
    ...
    PVECTOR_FOR_EACH (subtable, pvec) {
        pvector_change_priority(pvec, subtable, subtable->hit_cnt);//优先级
        subtable->hit_cnt = 0;
    }
    pvector_publish(pvec);
}
 ...
 * Returns true if all miniflows found a corresponding rule. */
 ...
 函数名称  :  dpcls_lookup
 功能描述  :  dpcls rule查询
 			  1.通过classifier查找掩码表，如果所有的报文都找到了匹配的子流表，将流表插入缓存中，
 			                        并且将报文加入flow->batches
...
#define MAP_BITS (sizeof(uint32_t) * CHAR_BIT)
    ...
    /*dpcls 掩码子表*/
    struct dpcls_subtable *subtable;    /*dpcls 掩码子表*/
    /*keys_map所有位都置1*/
