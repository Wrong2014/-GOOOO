...
main(int argc, char *argv[])
{
    ...
    struct unixctl_server *unixctl;/*unixctl服务*/
    char *remote;
    ...
    ovsthread_id_init();/*线程id初始化*/

    dns_resolve_init(true);  /*  空函数 toknow  */
    ...
    remote其实是一个socket文件地址，由 ovsdb-server服务端绑定监听时产生，
        作用类似于网络socket的Ip+Port地址，
    ...
    */
    ...
    while (!exiting) {
        /*运行内存监视器，客户端调用memory_should_report()。
         此函数以及该模块的接口的剩余部分，仅被一个线程调用*/
        memory_run();
        ...
        */
        ...
        /*执行在netdev_classes上定义的每个netdev_classs实体，调用他们的run()，
        会调用内核模块openvswitch.ko，在内核中添加虚拟网卡
        比如我们项目中的eth-rep口和virtio-net-rep口*/
        netdev_run();
