...
/*******************************************************************************
 ...
 功能描述  :    连接数据库ovsdb, 并从中读取配置信息
                主要是初始化网桥bridge模型，通过remote地址，
                来从OVSDB服务端获取配置信息来实现网桥初始化配置
                IDL，即接口定义语言，一般用于RPC（远程过程调用）。
            这里可以理解为用于vswitch和OVSDB服务之间的交互调用，
            一些信息就需要不断从OVSDB获取，如网桥信息
 输入参数  :  
 输出参数  :    
 返 回 值  :     无 
*******************************************************************************/
...
bridge_reconfigure(const struct ovsrec_open_vswitch *ovs_cfg)
{
    ...
    HMAP_FOR_EACH_SAFE (br, next, node, &all_bridges) {
        if (!br->ofproto) {
            ...
            /* 如果是新加的brdige会分配一个ofproto,
                实际分配oproto_dpif，创建br对应的backer*/
    ...
    HMAP_FOR_EACH (br, node, &all_bridges) {
        ...
        HMAP_FOR_EACH (port, hmap_node, &br->ports) {
            ...
            Interface: 连接到 Port 的网络接口设备。
            在通常情况下，Port 和 Interface 是一对一的关系, 
            只有在配置 Port 为 bond 模式后，Port 和 Interface 是一对多的关系。
            */
            /* 端口配置 */
            port_configure(port);
...
/*******************************************************************************
 ...
 功能描述  :    删除of层网桥，结构体ofproto代表一个ovs网桥，
                其含有of表、meter表、port、连接管理connmgr等属性
 输入参数  :    
 输出参数  :    
 返 回 值  :     无
*******************************************************************************/
...
/*******************************************************************************
 ...
 ovs-vsctl add-port br0 vx2 -- set interface vx2 type=vxlan options:~~~
*******************************************************************************/
static void
...
bridge_run__(void)
{
    struct bridge *br;/*网桥结构，面对用户*/
    struct sset types;
    const char *type;

    /* Let each datapath type do the work that it needs to do. */
    sset_init(&types);
    ofproto_enumerate_types(&types);/*获取of层的datapath 类型*/
    SSET_FOR_EACH (type, &types) {
        /*让每一个类型的datapath运行，ovs-netdev类型的ofproto*/
        ofproto_type_run(type);/*启动所有类型dp的ofproto*/
    }
    sset_destroy(&types);
