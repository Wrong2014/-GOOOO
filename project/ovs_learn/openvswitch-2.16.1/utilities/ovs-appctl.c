...
main(int argc, char *argv[])
{
    ...
    set_program_name(argv[0]);/*设置程序的name ovs-appctl*/

    /* Parse command line and connect to target. */
    target = parse_command_line(argc, argv);
    client = connect_to_target(target);/*起客户端，打开unix_socket文件，链接*/

    /* Transact request and process reply. */
    cmd = argv[optind++];/*ovs-appctl 后面的命令，指向dpcl/add-flow*/
    cmd_argc = argc - optind;
    cmd_argv = cmd_argc ? argv + optind : NULL;/*偏移到剩余参数位置*/
