...
ofproto_init(const struct shash *iface_hints)
{
    ...
    /*of层class注册ofproto_dpif_class 赋值给ofproto_classes，
      注册了操作函数库才可以创建ofproto，
...
ofproto_class_find__(const char *type)
{
    ...
    for (i = 0; i < n_ofproto_classes; i++) {
        ...
        sset_init(&types);/*hmap初始化*/
        class->enumerate_types(&types);
        found = sset_contains(&types, type);/*查找对应类的of*/
        sset_destroy(&types);
...
/*******************************************************************************
 ...
 如果dp类型为netdev，查找对应class，申请ofproto资源
    ...
    OVS_EXCLUDED(ofproto_mutex)
{
    ...
    *ofprotop = NULL;
    ...
    class = ofproto_class_find__(datapath_type);/*根据类型查找对应的class*/
    if (!class) {
