...
/*******************************************************************************
 ...
 功能描述  :    启动某类型的dp的ofproto, struct ofproto_class type_run
            1.启动对应backer类型的交换机，同类型交换机只有一个数据面，
              dpdk类型netdev的backer最终启动pmd线程开始poll收包 
...
{
    /*相同类型数据面共享一个backer，管理dp资源，
       dpdk类型只有一个netdev类型的backer，同类型所有网桥共享一个backer*/
    struct dpif_backer *backer;

    /*根据datapath type查找backer，即相同的datapath type只有一个backer，
       dpdk是netdev类型的backer*/
    ...
    if (!backer->recv_set_enable && !ofproto_get_flow_restore_wait()) {
        ...
        backer->need_revalidate = REV_RECONFIGURE;/* 配置变更 使flow生效*/
    }
    ...
    if (backer->need_revalidate) {
        struct ofproto_dpif *ofproto;/*of层dpif接口*/
...
open_dpif_backer(const char *type, struct dpif_backer **backerp)
{
    ...
    sset_init(&names);
    dp_enumerate_names(type, &names);/*根据类型netdev获取本类型下所有dp name*/
...
/*******************************************************************************
 ...
 功能描述  :    查找指定table的流表，默认是table 0，用一个循环去遍历每一个table，
            然后知道找到匹配的rule
