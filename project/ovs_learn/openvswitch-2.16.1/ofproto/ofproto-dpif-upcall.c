                    ...
                    uint32_t n_revalidators_)
{
    if (udpif && n_handlers_ && n_revalidators_) {
        ...
        dpif_enable_upcall(udpif->dpif);/* upcall上送使能 */
                  ...
                  uint32_t n_revalidators_)
{
    ...
    if (!udpif->handlers && !udpif->revalidators) {
        ...
        /* upcall dpif接口 启动handler和revaliadtor线程 */
...
udpif_revalidator(void *arg)
{
    ...
    struct udpif *udpif = revalidator->udpif;    /*upcall dp 接口*/
    ...
    revalidator->id = ovsthread_id_self();//本线程id
    for (;;) {
        if (leader) {
            uint64_t reval_seq;
            ...
            //获取datapath中flow总量 并更新到udpif中去
            n_flows = udpif_get_n_flows(udpif);
            udpif->max_n_flows = MAX(n_flows, udpif->max_n_flows);/*更新flow 总量*/
            udpif->avg_n_flows = (udpif->avg_n_flows + n_flows) / 2;/*更新平均flow总量 toknow*/
            ...
            if (!udpif->reval_exit) {
                ...
                /*创建dump flow的结构 描述一次dump流表，
                  只需第一个revalidator线程创建一次，所有revalidator线程共用*/
        ...
        if (leader) {
            ...
            //如果dump时间大于2000ms，则flow_limit需要除duration等于1000ms的倍数
            ...
            latch_wait(&udpif->exit_latch);//将exit_latch插入poll_loop节点
            latch_wait(&udpif->pause_latch);//将pause_latch插入poll_loop节点

            /* 调用poll函数，获取poll_loop节点上发生的事件，
              如果有事件发生，则继续执行，否则就等待超时后再继续执行。*/
            poll_block();
...
ukey_lookup(struct udpif *udpif, const ovs_u128 *ufid, const unsigned pmd_id)
{
    ...
    struct cmap *cmap = &udpif->ukeys[idx].cmap;/*全局ukey cmap*/
...
{
    uint64_t odp_actions_stub[1024 / 8];/*记录actions*/
    struct ofpbuf odp_actions = OFPBUF_STUB_INITIALIZER(odp_actions_stub);/*ofbut 描述 action*/

    struct udpif *udpif = revalidator->udpif;/*upcall结构*/

    /*dump线程描述一次dump过程，每个线程会创建自己的结构*/
    struct dpif_flow_dump_thread *dump_thread;
    uint64_t dump_seq, reval_seq;
    bool kill_warn_print = true;
    unsigned int flow_limit;/*flow限制*/
    ...
    /*创建struct dpif_flow_dump_thread结构，
       每个revalidator线程都会创建自己的struct dpif_flow_dump_thread结构*/
    ...
    for (;;) {
        ...
        for (f = flows; f < &flows[n_dumped]; f++) {
            long long int used = f->stats.used;/*获取flow最近命中时间*/
            struct recirc_refs recircs = RECIRC_REFS_EMPTY_INITIALIZER;
            struct dpif_flow_stats stats = f->stats;
            /*对flow要执行的操作op*/
            enum reval_result result;
            struct udpif_key *ukey;
            bool already_dumped;/*flow已经dump过*/
            int error;
