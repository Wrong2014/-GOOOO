...
/*
vm2上的ens8收到vxlan报文后，查找fast path失败，
上送slow-path，在函数terminate_native_tunnel中根据外层报文信息查找是否有匹配的tunnel，
如果有将设置action为OVS_ACTION_ATTR_TUNNEL_POP，出端口为tunnel口。
...
/*1.判断本地隧道终结是否使能
2.如果使能则根据报文带来的信息，查找隧道终结表是否可以命中
3.命中则进行tunnel_pop动作，剥除隧道头*/
static void
    ...
    if (xport->is_tunnel) {
        struct in6_addr dst;
          ...
          * matches, while explicit set actions on tunnel metadata are.
          保存隧道元数据，这样由于逻辑(隧道)端口所做的更改对于任何进一步的匹配都是不可见的，
          而对隧道元数据的显式设置操作是可见的。
          */
